...
export function JobsView() {
  ...
  useEffect(() => {
    ...
    let cancelled = false
    const reload = () => {
      api.listJobs(selectedContext, selectedNamespace ?? '').then((list) => {
        if (!cancelled) setJobs(list ?? [])
      })
    }
    reload()
    const unsub = onKubeChange('Job', (ctx) => {
  ...
  }, [selectedContext, selectedNamespace, setJobs])
  ...
  if (!selectedContext) {
    return (
      <div className="flex flex-1 items-center justify-center text-sm text-muted-foreground">
        Select a kubeconfig context to see jobs.
      </div>
    )
  }
          ...
          <tbody>
            {table.getRowModel().rows.length === 0 ? (
              <tr>
                ...
                >
                  No jobs{selectedNamespace ? ` in ${selectedNamespace}` : ''}.
                </td>
              </tr>
            ) : (
