...
export function IngressesView() {
  ...
  useEffect(() => {
    ...
    let cancelled = false
    const reload = () => {
      api.listIngresses(selectedContext, selectedNamespace ?? '').then((list) => {
        if (!cancelled) setIngresses(list ?? [])
      })
    }
    reload()
    const unsub = onKubeChange('Ingress', (ctx) => {
  ...
  }, [selectedContext, selectedNamespace, setIngresses])
  ...
  if (!selectedContext) {
    return (
      <div className="flex flex-1 items-center justify-center text-sm text-muted-foreground">
        Select a kubeconfig context to see ingresses.
      </div>
    )
  }
          ...
          <tbody>
            {table.getRowModel().rows.length === 0 ? (
              <tr>
                ...
                >
                  No ingresses{selectedNamespace ? ` in ${selectedNamespace}` : ''}.
                </td>
              </tr>
            ) : (
