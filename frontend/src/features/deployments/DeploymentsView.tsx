...
export function DeploymentsView() {
  ...
  useEffect(() => {
    ...
    let cancelled = false
    const reload = () => {
      api.listDeployments(selectedContext, selectedNamespace ?? '').then((list) => {
  ...
  }, [selectedContext, selectedNamespace, setDeployments])
  ...
  if (!selectedContext) {
    return (
      <div className="flex flex-1 items-center justify-center text-sm text-muted-foreground">
        Select a kubeconfig context to see deployments.
      </div>
    )
  }
          ...
          <tbody>
            {table.getRowModel().rows.length === 0 ? (
              <tr>
                ...
                >
                  No deployments{selectedNamespace ? ` in ${selectedNamespace}` : ''}.
                </td>
              </tr>
            ) : (
