...
export function SecretsView() {
  ...
  useEffect(() => {
    ...
    let cancelled = false
    const reload = () => {
      api.listSecrets(selectedContext, selectedNamespace ?? '').then((list) => {
        if (!cancelled) setSecrets(list ?? [])
      })
    }
    reload()
    const unsub = onKubeChange('Secret', (ctx) => {
  ...
  }, [selectedContext, selectedNamespace, setSecrets])
  ...
  if (!selectedContext) {
    return (
      <div className="flex flex-1 items-center justify-center text-sm text-muted-foreground">
        Select a kubeconfig context to see secrets.
      </div>
    )
  }
          ...
          <tbody>
            {table.getRowModel().rows.length === 0 ? (
              <tr>
                ...
                >
                  No secrets{selectedNamespace ? ` in ${selectedNamespace}` : ''}.
                </td>
              </tr>
            ) : (
